...
- **RLM (Robot Lifecycle Management) モード**
  ...
  チーム開発の **原本（マスターデータ）** となる特別な保管庫（リポジトリ）を作成・管理する **プロモーションマネージャー向けの、機能が限定されたモード** です。
...
- **Stash（一時退避）：作業を中断して別のタスクへ**
  「コミットするほどではないけど、急な修正依頼が入ったので一旦作業を中断したい」といった場合に、現在の変更内容を「一時退避」させることができます。

  1.  **退避**: `[変更]` カラムのヘッダーにある **[save]** アイコン（一時退避する）をクリックします。後で分かりやすいように簡単なメモを入力すると、変更内容（ステージング済み／未ステージングの両方）が `[一時退避中の作業]` リストに保存され、作業場はきれいな状態に戻ります。
  2.  **再開**: 別の作業が終わった後、`[一時退避中の作業]` リストから戻したい作業の右側にある **[unarchive]** アイコン（適用する）をクリックすると、退避した変更内容が作業場に復元されます。不要になった退避は **[delete_forever]** アイコンで削除できます。
